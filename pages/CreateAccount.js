...
const CreateAccountForm = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    (async () => {
      try {
        const response = await fetch(`/api/account/create?name=${name}&email=${email}&password=${password}`);
        if (response.ok) {
          const res = await response.json();
          props.setShow(false);
        } else {
          const errorMessage = await response.json()
          console.error('Erreur de création : ', errorMessage.message || '');
  ...
  React.useEffect(() => {
    setAvailableButton(name?.length > 0 && email?.length > 0 && password?.length > 0)
  }, [email, name, password]);

  return (
    <form onSubmit={handleSubmit}>
    <label className="my-2">Enter name:
    <input
      type="text"
      name="name"
    ...
    <label className="my-2">Enter email:
      <input
        required
        type="email"
        name="email"
      ...
      <label className="my-2">Enter password:
      <input
        required
...
{/* 
      ...
      <input disabled={!availableButton} className="btn btn-light my-3" type="submit" />
