...
const CreateAccountHandler = ({createAccount}) => {
  ...
  return (
    <div>
      <Button variant="success" className="mt-3" onClick={() => setShowAccountModal(true)}>
        Create a new account
      </Button>
      {showAccountModal && <CreateAccountModal show={showAccountModal} onHide={() => setShowAccountModal(false)} createAccount={createAccount}/>}
    </div>
  )
